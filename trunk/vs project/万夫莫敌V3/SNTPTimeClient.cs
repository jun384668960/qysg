...
namespace register_client
{
    ...
    public class SNTPTimeClient
    {
        ...
        static byte[] NTPData = new byte[NTPDataLength];
        ...
        public void Connect(AsyncCallback callback)
        {
            try
            {
                ...
                Initialize();
                TimeSocket.Send(NTPData, NTPData.Length, EPhost);

                UdpState state = new UdpState();
                state.ipEndPoint = EPhost;
                state.udpClient = TimeSocket;
                state.callback = callback;
                TimeSocket.BeginReceive(ReceiveCallback, state);
                //TimeSocket.EndReceive(ref EPhost);
                //NTPData = TimeSocket.Receive(ref EPhost);
                /*
                IPEndPoint localIpep = new IPEndPoint(
                    IPAddress.Parse("127.0.0.1"), Convert.ToInt32(TimePort));
                IPEndPoint remoteIpep = new IPEndPoint(IPAddress.Any, 0);
                UdpClient udpcRecv = new UdpClient(localIpep);
                NTPData = udpcRecv.Receive(ref remoteIpep);
                 * */
                if (!IsResponseValid())
                {
                    //MessageBox.Show("Invalid response from " + TimeServer);
