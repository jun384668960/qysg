...
namespace register_client
{
    ...
    {
        //开关
        public static bool g_bListen = true;                //是否启用非法检测
        public static bool g_bAutoAttack = false;           //
        public static bool g_loadprotect = true;            //是否使用Protect
        public static long StartFileSize = 2700830L;
        public static bool showStartFileSize = false;
        public static string g_CmdLine = "fuck you man! 1 2 3 4 5"; 
        public static int g_limitNum = 13;                  //正常情况下的多开现在
        public static int g_SpcTimeLimitNum = 13;            //特殊时段多开限制
        ...
        public static string SpcTimeStart = "19:57";        //特殊时段
        public static string SpcTimeEnd = "22:03";
        ...
        //记录已经开启的游戏PID
        public static Dictionary<int, Process> Dic_GameProcess = new Dictionary<int, Process>();
        ...
        public static string protectProssesId = "-1";
        ...
        private void LoginClient_Shown(object sender, EventArgs e)
        {
            try
            {
                ...
                Parameter = "del Update.cab SOLUpd3.pak SOLUpd4.pak Update.pak server.ini";
                p.Start();
                p.StandardInput.WriteLine(Parameter);
                p.StandardInput.WriteLine("exit");
                p.StandardInput.WriteLine("exit");
        ...
        public static void CloseAllGame(object ob)
        {
            ...
            {
                FlushProcessLib();
                foreach (var item in Dic_GameProcess)
                {
                    item.Value.Kill();
                }
                Dic_GameProcess.Clear();
        ...
        public static bool CheckStartFile(string file)
        {
            ...
            if (f.Length >= StartFileSize + 512000 || f.Length < StartFileSize - 512000)
            {
        ...
        public static void StartLoginGame()
        {
            try
            {
                ...
                if (Dic_GameProcess.Count >= limitNum)
                {
                ...
                //记录进程
                Dic_GameProcess.Add(newProcess.Id, newProcess);
            }
            catch
            {
        ...
        public static void FlushProcessLib()
        {
            ...
            else
            {
                ...
                foreach (var item in Dic_GameProcess)
                {
                    ...
                    if (!contain)
                    {
                        Dic_GameProcess.Remove(item.Key);
                        break;
                    }
                }
            }
        ...
        static private void ThreadListen(object ob)
        {
            ...
            while (g_thListenStop)
            {
                ...
                if (spcTime && Dic_GameProcess.Count > g_SpcTimeLimitNum)
                {
                    MessageBox.Show("IsSpcTime" + Dic_GameProcess.Count);
                    CloseAllGame(ob);
                }
                else
                {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (g_loadprotect)
            {
                ...
                foreach (var curProcess in procList)
                {
                    //if (curProcess.ProcessName.ToString() == protectProssesName && curProcess.Id.ToString() == protectProssesId)
        ...
        private void LoadProtectPresses()
        {
            ...
            //创建进程
            System.IO.Directory.SetCurrentDirectory(System.Environment.CurrentDirectory);
            System.Diagnostics.ProcessStartInfo psi = new System.Diagnostics.ProcessStartInfo(protectfile, protectLine + " " + StartFile + " " + Process.GetCurrentProcess().Id.ToString());
            psi.RedirectStandardOutput = true;
            psi.UseShellExecute = false;
            Process newProcess = Process.Start(psi);

            protectProssesId = newProcess.Id.ToString();
        }
        ...
        {
            g_strVersion = str.Substring(0, g_revStr.IndexOf("版本"));

            //MessageBox.Show(g_strLink);
            //check version
            if (g_strVersion != g_oldVersion)
            {
                //MessageBox.Show("g_strVersion:" + g_strVersion + "!= g_oldVersion:" + g_oldVersion);
                //去更新
                StartUpdateGame();
                //MessageBox.Show("StartUpdateGame");
                CloseAllGame(this);
